...
void GNTdebug()
{
    ...
    if (retarget == DIFF_DGW)
    {
        ...
        LogPrintf("Newyorkcoin retargetted using: DGW-v3 difficulty algo \n");
        return;
    }
    // Retarget using PPC
    else if (retarget == DIFF_PPC)
    {
        ...
        LogPrintf("Newyorkcoin retargetted using: PPC difficulty algo \n");
        return;
    }
    // Retarget using Terminal-Velocity
    // debug info for testing
    LogPrintf("Terminal-Velocity retarget selected \n");
    LogPrintf("Newyorkcoin retargetted using: Terminal-Velocity difficulty curve \n");
    return;
}
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    // Reserved for Swap from Initial Newyorkcoin Chain Launch
    else if(nHeight > nReservePhaseStart) {
