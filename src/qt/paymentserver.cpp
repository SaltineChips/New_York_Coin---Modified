...
const int NEWYORKCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString NEWYORKCOIN_IPC_PREFIX("Newyorkcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(NEWYORKCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(NEWYORKCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(NEWYORKCOIN_IPC_CONNECT_TIMEOUT);
