...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NYC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NYC
    // Mac: ~/Library/Application Support/NYC
    // Unix: ~/.NYC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NYC";
#else
    fs::path pathRet;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Newyorkcoin.conf";
